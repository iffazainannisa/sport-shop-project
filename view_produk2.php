...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">Product
    </section>

    <!-- Main content -->
    <section class="content">
      <?php
        $query5 = " SELECT * FROM produk INNER JOIN sub_kategori ON produk.idsub_kategori=sub_kategori.idsub_kategori WHERE produk.idsub_kategori=".$idsub;
        // Execute the query
        ...
        if (!$result5){
          ...
          while ($row5 = $result5->fetch_object()){
            echo'<div class="w3-quarter" style="width:20%">';
            echo '<div class="w3-card">';
            echo'<img src="images/'.$row5->file_gambar.'"'; 
            echo 'alt="Alps" style="width:100%">';
            echo '<div class="w3-container w3-center">';
            echo '<h4><b>'.$row5->nama_produk.'</b></h4>';
            echo'<p class="harga">Rp'.$row5->harga.'</p>';
            $query4 = " SELECT * FROM pegawai WHERE idpegawai=".$row5->idpegawai;
            $result4 = $db->query( $query4 );
                
            if ($result4) {
              $rows4 = $result4->num_rows;
              if ($rows4 == 1) {
                $profil = $result4->fetch_object();
                echo'<p><i class="far fa-calendar-alt"></i> &nbsp;'.format_indo($row5->last_update).' &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;<span><i class="fas fa-user"></i> &nbsp;'.$profil->nama_lengkap.'</span></p>';
              }
            }else {
              die ("Could not query the database: <br />". $db->error);
            }
            echo '<a href="detail_produk.php?pid='.$row5->idproduk.'"<button type="button" class="btn btn-primary" style="margin-bottom:15px;">Detail Produk</button></a>';
            echo '</div>';
            echo '</div>';
            echo '</div>';
          }
        ...
        function format_indo($date){
          $BulanIndo = array("Jan", "Feb", "Mar", "Apr", "Mei", "Juni", "Juli", "Agt", "Sept", "Okt", "Nov", "Des");
